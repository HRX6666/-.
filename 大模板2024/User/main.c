...
unsigned char i;
float Temp;
unsigned char Seg_Disp_Mode;//数码管显示模式 0-数据显示界面 1-参数界面
float Temp_Max=30;
float Temp_Max_crol;
float Temp_Min=20;
float Temp_Min_crol;
	...
	switch(Key_Val)
	{
		...
		case 5:
			if(Key_Down)
			{
				...
				{
					if(i==0)
					Temp_Max_crol=Temp_Max;
					else if(i==1)
					Temp_Min_crol=Temp_Min;
					else
						i=0;
				}
			}
...
void Seg_Proc()
{
	...
	switch(Seg_Disp_Mode)
	{
		...
		case 1:
			Seg_Buf[0]=12;
			Seg_Buf[3]=(unsigned char)Temp_Max/10;
			Seg_Buf[4]=(unsigned char)Temp_Max%10;
			Seg_Buf[5]=10;
			Seg_Buf[6]=(unsigned char)Temp_Min/10;
			Seg_Buf[7]=(unsigned char)Temp_Min%10;
		break;
