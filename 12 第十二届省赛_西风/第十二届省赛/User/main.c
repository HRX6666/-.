...
float Temperature;//实时温度变量     显示温度的功能
...
void Key_Proc()
{
	...
	switch(Key_Down)
	{
		...
		case 8://参数自减按键
			if(Seg_Disp_Mode == 1) //当前处于温度参数设置界面
			{
				if(--Temperature_Params == 255) //限制温度下限为0，虽然没说但是需要设置限制一下否则显示有误
...
void Seg_Proc()
{
	...
	switch(Seg_Disp_Mode)
	{
		case 0://温度显示界面
			Seg_Buf[0] = 11;//标识符C
			Seg_Buf[4] = (unsigned char)Temperature / 10 % 10;//除10再取余10
			Seg_Buf[5] = (unsigned char)Temperature % 10;
			Seg_Buf[6] = (unsigned int)(Temperature * 100) / 10 % 10;//扩大100倍
